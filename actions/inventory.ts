...
export const syncProductStock = async (productId: string, newStock: number, reason: string, platform: (typeof PLATFORMS)[number], sku: string, description?: string) => {
  ...
  return authGuard<{ success: boolean; message: string }>('SYNC_PRODUCT_STOCK', '/products', async (userId) => {
    // Redis distributed lock to prevent race conditions
    return await withDistributedLock(productId, async () => {
...
export const forceSyncAllProducts = async () => {
  return authGuard<{ success: boolean; message: string }>('FORCE_SYNC_ALL_PRODUCTS', '/products', async (userId) => {
    // 🛡️ Scope the lock to the specific user so they don't block others!
    const userLockKey = `lock:force-sync:${userId}`;

    // Respect the Redis lock
    return await withDistributedLock(userLockKey, async () => {
      ...
      try {
        ...
        const [hasProducts, hasActiveStore] = await Promise.all([Product.exists({ userId }), Store.exists({ userId, isSyncEnabled: true })]);

        if (!hasProducts) return { success: false, message: 'No products found' };
        if (!hasActiveStore) return { success: false, message: 'No active stores found' };
