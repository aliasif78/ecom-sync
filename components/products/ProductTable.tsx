...
const ProductTable = ({ products, isSyncing }: Props) => {
  ...
  const [disabledDeleteId, setDisabledDeleteId] = useState<string | null>(null);
  ...
  const handleDelete = async (id: string) => {
    if (id === disabledDeleteId) return;

    if (confirm('Are you sure that you want to delete this product?')) {
      setDisabledDeleteId(id);
      const { success, message } = await deleteProduct(id);

      if (success) toast.success(message);
      else toast.error(message);

      setDisabledDeleteId(null);
    }
  };
          ...
          <tr key={product._id} className="group transition-all duration-300 hover:bg-white/5">
            ...
            <td className="px-8 py-5 text-right text-sm font-medium whitespace-nowrap">
              <div className="flex items-center justify-end gap-3">
                ...
                <ActionButton icon={<Icons.Delete />} onClick={() => handleDelete(product._id)} variant="danger" title="Delete Product" disabled={disableSync || disabledDeleteId === product._id} />
